...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            doc = {}
            for oldfieldname, newfieldname in fields.items():
                item = line[oldfieldname]
                if oldfieldname == "rdf-schema#label":
                    item = re.sub("\(.*?\)", "", item)
                    item = item.strip()
                    doc[newfieldname] = item
                if oldfieldname == "name":
                    if not item.isalnum() or item == "NULL":
                        item = line["rdf-schema#label"]
                    doc[newfieldname] = item
                else:
                    if item == "NULL":
                        item = None
                    ...
                    if newfieldname in ["family","class","phylum","order","kingdom","genus"]:
                        if not "classification" in doc:
                            doc["classification"] = {}
                        doc["classification"][newfieldname]=item
                    elif item <> None and newfieldname == "synonym":
                        itemarray = item.strip("{}").split("|")
                        doc[newfieldname]=itemarray
                    else:
                        doc[newfieldname]=item
