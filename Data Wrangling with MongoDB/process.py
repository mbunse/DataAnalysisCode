...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in soup.find("table", class_="dataTDRight").find_all("tr",class_="dataTDRight"):
            datarow = {}
            col = row.find_all("td")
            if col[1].get_text()=="TOTAL": continue
            datarow["courier"]=info["courier"]
            datarow["airport"]=info["airport"]
            datarow["year"]=int(col[0].get_text())
            datarow["month"]=int(col[1].get_text())
            datarow["flights"]={"domestic": locale.atoi(col[2].get_text()), "international": locale.atoi(col[3].get_text())}
            print datarow
            data.append(datarow)
